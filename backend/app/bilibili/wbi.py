...
很多 B站 web API（player/wbi/v2、conclusion/get、search 等）需要 WBI 签名
才能拿到完整数据。不签名时常常返回阉割数据或 stale/错配的内容——尤其是
字幕接口，没签名拿到的字幕 URL 很可能指向另一个视频的字幕。

签名流程（B站 web 客户端公开实现）：
  1. /x/web-interface/nav 返回 wbi_img.{img_url, sub_url}
  2. 取 URL 文件名（去 path、去扩展名）得到 imgKey 和 subKey
  3. 把 imgKey + subKey 拼起来，按公开常量 MIXIN_KEY_ENC_TAB 重排，取前 32 字符 = mixin_key
  4. 给参数加 wts（unix 时间戳），按 key 字典序排序，URL-encode 拼接
  5. 拼上 mixin_key 求 md5 = w_rid

img_key/sub_key 一天换一次。我们做了 6 小时缓存避免每次都打 nav。
"""
