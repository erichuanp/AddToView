...
<template>
  <div v-if="data && data.count > 0" class="glass px-3 py-1.5 mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
    <div>
      <span class="text-soft mr-1">剩余总时长</span>
      <strong class="tabular-nums text-sm">{{ data.remaining_total_pretty }}</strong>
    </div>
    <div v-if="data.top_owners_by_time.length > 0" class="text-soft ml-auto truncate">
      最耗时 UP：
